...
═══════════════════════════════════════════════════════════ */
function AgreementCatalogTab({ items, isLoading, search }) {
  const q = search.toLowerCase();
  const filtered = items.filter(item =>
    [item.product_name, item.barcode, item.sku, item.vendor_name]
      .some(v => String(v ?? "").toLowerCase().includes(q))
  );
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-50">
              ...
              <TableHead className="text-xs font-semibold text-gray-600">Barcode</TableHead>
              <TableHead className="text-xs font-semibold text-gray-600">Vendor</TableHead>
              <TableHead className="text-xs font-semibold text-gray-600">Agreement</TableHead>
              <TableHead className="text-xs font-semibold text-gray-600 text-right">Vendor Price</TableHead>
              <TableHead className="text-xs font-semibold text-gray-600 text-right">Unit Price</TableHead>
              <TableHead className="text-xs font-semibold text-gray-600 text-right">MOQ</TableHead>
              <TableHead className="text-xs font-semibold text-gray-600 text-right">Lead time</TableHead>
              <TableHead className="text-xs font-semibold text-gray-600">Flags</TableHead>
              <TableHead className="text-xs font-semibold text-gray-600 text-center">Active</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={10} className="text-center py-12 text-gray-400 text-sm">
                  {search ? "No items match your search." : "No agreement products yet. Upload a vendor agreement PDF."}
                </TableCell>
              </TableRow>
            ) : (
              filtered.map(item => (
                <TableRow key={item.id} className="hover:bg-gray-50 transition-colors">
                  <TableCell>
                    ...
                    {item.mapped_product_name && item.mapped_product_name !== item.product_name && (
                      <p className="text-[10px] text-blue-500">→ {item.mapped_product_name}</p>
                    )}
                  </TableCell>
                  <TableCell className="text-xs font-mono text-gray-500">{item.barcode || "—"}</TableCell>
                  <TableCell className="text-xs text-gray-600">{item.vendor_name}</TableCell>
                  <TableCell className="text-xs font-mono text-gray-400">{item.agreement}</TableCell>
                  <TableCell className="text-right tabular-nums text-xs text-gray-700">
                    ₹{fmt(item.vendor_price)}<br />
                    <span className="text-[10px] text-gray-400">/{item.purchase_unit || "carton"}</span>
                  </TableCell>
                  <TableCell className="text-right tabular-nums text-xs text-gray-700">
                    ₹{fmt(item.price_per_base_unit)}<br />
                    <span className="text-[10px] text-gray-400">/{item.base_unit || "unit"}</span>
                  </TableCell>
                  <TableCell className="text-right tabular-nums text-xs">{item.moq ?? "—"}</TableCell>
                  ...
                  <TableCell>
                    <div className="flex flex-col gap-0.5">
                      {item.is_new_product && (
                        <span className="px-1.5 py-0.5 rounded text-[9px] font-semibold bg-green-100 text-green-700">NEW</span>
                      )}
                      {item.is_multi_vendor && (
                        <span className="px-1.5 py-0.5 rounded text-[9px] font-semibold bg-purple-100 text-purple-700">MULTI</span>
                      )}
                      {item.is_mapped && (
                        <span className="px-1.5 py-0.5 rounded text-[9px] font-semibold bg-blue-100 text-blue-700">MAPPED</span>
                      )}
                    </div>
                  </TableCell>
                  <TableCell className="text-center">
                    <span className={`w-2 h-2 rounded-full inline-block ${item.agreement_active ? "bg-green-400" : "bg-gray-300"}`} />
                  </TableCell>
                </TableRow>
              ))
            )}
