...
function InlineQCRow({ item, onSaved }) {
  ...
  const handleSave = async () => {
    if (isOver || (!accepted && !rejected)) return;
    setSaving(true);
    try {
  ...
  return (
    <div
      className={`grid grid-cols-[1fr_auto] gap-x-4 gap-y-1 px-5 py-4 border-b border-gray-50 last:border-0
        ...
        <div className="flex flex-wrap gap-4 mt-1.5 text-xs">
          <span className="text-gray-500">
            ...
            <span className="font-mono text-gray-700">
              {item.snapshot_barcode || "—"}
            </span>
          </span>
          <span className="text-gray-500">
            Item ID:{" "}
            <span className="font-mono text-gray-600">{item.grn_item_id}</span>
          </span>
          <span className="text-gray-500">
            Received:{" "}
      ...
      {isCompleted ? (
        ...
        <div className="flex items-end gap-2 shrink-0">
          <div className="grid gap-1">
            <Label className="text-[10px] text-gray-400 text-center font-medium">
              Accept
            </Label>
            <Input
              type="number"
              ...
              className={`w-20 h-8 text-sm text-center tabular-nums border-gray-300
          ...
          <Button
            ...
            onClick={handleSave}
            disabled={
              saving || isOver || Number(accepted) + Number(rejected) === 0
            }
